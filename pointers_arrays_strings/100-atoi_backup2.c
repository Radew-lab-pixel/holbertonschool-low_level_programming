...
int char2int ( char *str)
{
	...
	*/
	...
	{	
		...
		{
			if (*str == '-') /* if str[count] is negative sign */
			{
				sign = -1 * sign; /* multiply current sign factor by -1 */
			}	
			else if (((*str - '0') >= 0) && ((*str - '0') <= 9)) /* is str between 0 and 9 , converted to integer */ 
			{
				result = result * 10 + (*str - '0' ); /* multiple each numerical by 10 if more digit found , -'0' is char  to integer */
			}
			else /*other character including + sign */
			{
				if ((*str - 1) - '0') >= 0) && ((str[count - 1] - '0') <= 9)) /* found the previous str[count] is numerical */
				{
